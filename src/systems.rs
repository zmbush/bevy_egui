...
pub fn process_output_system(
    ...
    for mut context in contexts.iter_mut() {
        ...
        if let Some(_) = context.window {
            let mut set_icon = || {
                commands.entity(context.render_target).insert(
                    egui_to_winit_cursor_icon(platform_output.cursor_icon)
                        .map(bevy_render::view::cursor::CursorIcon::System)
                        .unwrap_or(bevy_render::view::cursor::CursorIcon::System(
